...
class PrepareDirectories(SimpleTask):
    ...
    def process(self, item):
        ...
        escaped_item_name = item_name.replace(':', '_').replace('/', '_') \
            .replace('~', '_')
        hashed_item_name = hashlib.sha1(escaped_item_name.encode('utf-8')) \
            .hexdigest()
...
class WgetArgs(object):
    def realize(self, item):
        ...
        if item_type == 'repos':
            for repo in item_value.split(';'):
                repo = repo.strip()
                wget_args.extend(['--warc-header',
                                  'github-project-main-page: {}'.format(repo)])
                wget_args.append('https://github.com/{}'.format(repo))
        else:
...
project = Project(
    ...
    project_html = '''
    <img class="project-logo" alt="logo" src="https://www.archiveteam.org/images/3/3d/GitHub_logo.png" />
    <h2>github.com <span class="links"><a href="https://github.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/githubdisco/">Leaderboard</a></span></h2>
    '''
)
